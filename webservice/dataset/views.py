...
class DynamicTraining(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            dataset = Dataset.objects.get(filename=request.POST['selected'])
        except:
            print('dataset does not exist')
            return HttpResponse('Failed to fetch dataset', status=404)



        model = dynamicTraining.DynamicTraining(request, str(dataset.file), request.POST['selected'], request.POST['epochs'], request.POST['epochs_steps'])
...
class ChunkedUploadView(ListModelMixin, RetrieveModelMixin,
    ...
    def on_completion(self, chunked_upload, request) -> Response:
        ...
        date = [year, month, day]
        root_dir = os.path.join(MEDIA_ROOT, 'chunked_uploads')
        count = 0
        current = root_dir
        ...
        # Search for files with the given extension in the current directory
            ...
            # If a match is found, add the full path to the list
        ...
        for root, dirs, files in os.walk(root_dir):
            ...
            if count < 3 and date[count] in dirs:
                current = os.path.join(current, str(date[count]))
                print(dirs)
                print(current)
                print(date[count])
                count += 1
        ...
        """
        file_done = glob.glob(current + '/*.done')
        for file in files:

            filepath = str(file)
            dirname, filename = os.path.split(filepath)
            base_name, extension = os.path.splitext(filename)
            new_filename = base_name + '.zip'
            new_file_path = os.path.join(dirname, new_filename)
            os.rename(filepath, new_file_path)
            file = str(new_file_path)
            dataset = Dataset.create(file=file, filename=request.POST['filename'])
            dataset.save()
