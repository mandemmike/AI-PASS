...
class TestParseDataset(TestCase):
    ...
    def test_ParseInfoFromFile_SplitDataframe_CheckIfEachFeatureSplit_Successful(self):
        ds_path = '../webservice/dataset/test_data/'
...
class TestUtkFaceDataGenerator(TestCase):
    ...
    def test_generate_split_indexes(self):
        df_count = 23708
        expected_train_percentage = 64  # 64% training dataset
        expected_valid_percentage = 16  # 16% validation dataset
        expected_test_percentage = 20  # 20% test dataset

        self.training_set, self.valid_set, self.test_set = UtkFaceDataGenerator.generate_split_indexes(
            UtkFaceDataGenerator)

        actual_train_perc = round((len(self.training_set) / df_count) * 100)
        actual_valid_perc = round((len(self.valid_set) / df_count) * 100)
        actual_test_perc = round((len(self.test_set) / df_count) * 100)
        print('print: ', actual_train_perc)
    ...
    def test_preprocess_image_shape(self):
        expected = (64, 64, 3)
        path = '../webservice/dataset/test_data/23_0_3_20170119163843478.jpg.chip.jpg'
